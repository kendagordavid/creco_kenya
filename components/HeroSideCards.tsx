    ...
    <div className="grid h-full gap-4 sm:grid-cols-2 lg:grid-cols-1">
      {cards.map((card, index) => (
        <Link
          key={card.href}
          href={card.href}
          className="group flex min-h-[10.5rem] flex-col overflow-hidden rounded-[1.25rem] bg-white shadow-md ring-1 ring-black/5 no-underline transition hover:-translate-y-0.5 hover:shadow-lg lg:min-h-0 lg:flex-1"
        >
          <div className="relative h-28 overflow-hidden sm:h-24 lg:h-[42%]">
            <img
          ...
          <div
            className={`flex flex-1 flex-col justify-between gap-3 px-4 py-4 text-white ${toneClasses[card.tone as keyof typeof toneClasses] ?? toneClasses.green}`}
          >
